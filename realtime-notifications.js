...
let auth,db,profile=null,currentUid=null;
let unsubscribeAll=[];
let initializedCollections=new Set();
let notifications=[];

initializeRealtime();
installModuleReadCapture();
...
async function initializeRealtime(){
  try{
    ...
    authModule.onAuthStateChanged(auth,async user=>{
      stopListeners();profile=null;currentUid=user?.uid||null;initializedCollections.clear();
      notifications=loadNotifications();updateNotificationState();
      if(!user)return;
      try{
        const snap=await firestore.getDoc(firestore.doc(db,'apps',APP_NAMESPACE,'communities',COMMUNITY_ID,'users',user.uid));
        profile=snap.exists()?{uid:user.uid,...snap.data()}:{uid:user.uid,role:'resident'};
      }catch{profile={uid:user.uid,role:'resident'};}
    ...
    const unsubscribe=firestore.onSnapshot(liveQuery,snapshot=>{
      ...
      if(!isInitial){
        snapshot.docChanges().forEach(change=>{
          ...
          if(canSee(module,item,user))addNotification(module,item);
        });
      }
...
function installModuleReadCapture(){
  document.addEventListener('click',event=>{
    ...
    if(!button)return;
    const module=identifyModule(button.textContent);
    if(module)markModuleRead(module);
  },true);
  window.addEventListener('neighbor:module-opened',event=>{const module=event.detail?.module;if(module)markModuleRead(module);});
}

function markModuleRead(module){
  let changed=false;
  notifications=notifications.map(item=>{
    if(item.module===module&&!item.read){changed=true;return {...item,read:true};}
    return item;
  });
  if(!changed)return;
  persistNotifications();updateNotificationState();
}

function updateNotificationState(){
  const unread=notifications.filter(item=>!item.read).length;
  const counts={};COLLECTIONS.forEach(module=>counts[module]=notifications.filter(item=>item.module===module&&!item.read).length);
  const badge=document.querySelector('#neighborBadge');
  if(badge){badge.textContent=unread>99?'99+':String(unread);badge.classList.toggle('visible',unread>0);}
  document.title=unread>0?`(${unread}) Neighbor · Terra Lugo`:'Neighbor · Terra Lugo';
  window.dispatchEvent(new CustomEvent('neighbor:notification-state',{detail:{unread,counts}}));
}
...
function updateLiveCounters(module,items){
  ...
  document.querySelectorAll('.action-card').forEach(card=>{
    const text=card.textContent.toLowerCase();if(!(ALIASES[module]||[]).some(alias=>text.includes(alias)))return;
    const detail=card.querySelector('small');if(!detail)return;
...
function refreshOpenModule(module){
  const dialog=document.querySelector('#featureDialog');if(!dialog?.open)return;
  const currentTitle=document.querySelector('#dialogTitle')?.textContent?.toLowerCase()||'';
  if(!(ALIASES[module]||[]).some(name=>currentTitle.includes(name.split(' ')[0])))return;
  const trigger=findModuleTrigger(module);if(trigger)setTimeout(()=>trigger.click(),80);
}

function createNotificationUi(){
  if(document.querySelector('#neighborBell'))return;
  const style=document.createElement('style');style.textContent=`.neighbor-bell{position:fixed;right:18px;bottom:86px;z-index:50;width:50px;height:50px;border:0;border-radius:50%;background:#071522;color:white;font-size:22px;box-shadow:0 10px 28px rgba(0,0,0,.24);display:none;align-items:center;justify-content:center}.neighbor-bell.visible{display:flex}.neighbor-badge{position:absolute;right:-3px;top:-3px;min-width:21px;height:21px;padding:0 5px;border-radius:11px;background:#dc2626;color:#fff;font:700 12px/21px Inter,sans-serif;display:none}.neighbor-badge.visible{display:block}.neighbor-toast{position:fixed;left:16px;right:16px;top:calc(env(safe-area-inset-top) + 14px);z-index:100;background:#fff;border:0;border-radius:16px;padding:14px 16px;box-shadow:0 16px 45px rgba(0,0,0,.24);display:flex;gap:12px;align-items:center;text-align:left}.neighbor-toast span{font-size:24px}.neighbor-toast strong,.neighbor-toast small{display:block}.neighbor-toast small{margin-top:3px;color:#52606d}.notification-row{width:100%;border:0;background:transparent;text-align:left;padding:0}.notification-row.unread strong:after{content:' •';color:#dc2626}.notification-meta{font-size:12px;color:#667085;margin-top:4px}.notification-actions{display:flex;justify-content:flex-end;gap:8px;margin-bottom:12px}`;document.head.appendChild(style);
  const bell=document.createElement('button');bell.id='neighborBell';bell.className='neighbor-bell';bell.type='button';bell.setAttribute('aria-label','Abrir notificaciones');bell.innerHTML='🔔<span class="neighbor-badge" id="neighborBadge">0</span>';bell.addEventListener('click',openNotificationCenter);document.body.appendChild(bell);
  const appShell=document.querySelector('#appShell');const syncVisibility=()=>bell.classList.toggle('visible',!appShell?.classList.contains('hidden'));syncVisibility();if(appShell)new MutationObserver(syncVisibility).observe(appShell,{attributes:true,attributeFilter:['class']});
}

function showToast(notification){
  document.querySelector('.neighbor-toast')?.remove();const toast=document.createElement('button');toast.type='button';toast.className='neighbor-toast';toast.innerHTML=`<span>${notification.icon}</span><div><strong>${escapeHtml(notification.title)}</strong><small>${escapeHtml(notification.detail)}</small></div>`;
  toast.onclick=()=>{toast.remove();markModuleRead(notification.module);openTargetModule(notification.module);};document.body.appendChild(toast);setTimeout(()=>toast.remove(),5000);
}

async function openNotificationCenter(){
  if('Notification'in window&&Notification.permission==='default')Notification.requestPermission().catch(()=>{});
  const dialog=document.querySelector('#featureDialog'),eyebrow=document.querySelector('#dialogEyebrow'),title=document.querySelector('#dialogTitle'),body=document.querySelector('#dialogBody'),form=document.querySelector('#dialogForm');if(!dialog||!body)return;
  form.onsubmit=null;eyebrow.textContent='Neighbor en vivo';title.textContent='Notificaciones';
  body.innerHTML=notifications.length?`<div class="notification-actions"><button id="markAllRead" type="button">Marcar leídas</button><button id="clearNotifications" type="button">Limpiar</button></div><div class="module-list">${notifications.map(notificationRow).join('')}</div>`:'<div class="empty-state">No hay notificaciones.</div>';
  if(!dialog.open)dialog.showModal();
  document.querySelector('#markAllRead')?.addEventListener('click',()=>{notifications=notifications.map(item=>({...item,read:true}));persistNotifications();updateNotificationState();openNotificationCenter();});
  document.querySelector('#clearNotifications')?.addEventListener('click',()=>{notifications=[];persistNotifications();updateNotificationState();openNotificationCenter();});
  document.querySelectorAll('[data-notification-id]').forEach(button=>button.addEventListener('click',()=>{const item=notifications.find(entry=>entry.id===button.dataset.notificationId);if(!item)return;markModuleRead(item.module);dialog.close();openTargetModule(item.module);}));
}
